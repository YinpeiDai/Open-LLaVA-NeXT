...
from minigrid.utils.data_preprocess.rm_prompt import (
    SYSTEM_PROMPT_NO_BELIEF, SYSTEM_PROMPT_FIRST_BELIEF, SYSTEM_PROMPT_ZEROTH_BELIEF)
...
def preprocess_llama3(
    ...
    for i, source in enumerate(sources):
        ...
        if setting == "none":
            ...
            system_prompt = SYSTEM_PROMPT_FIRST_BELIEF.replace("WIDTH", str(info["width"])).replace("HEIGHT", str(info["height"])).replace("WINSIZE", str(info["window_size"]))
        else:
