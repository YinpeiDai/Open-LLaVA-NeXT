...
from minigrid.utils.data_preprocess.finetuned_prompt_belief_as_input import (
...
def generate_data(
    ...
    for idx, data in enumerate(raw_data):
        ...
        if idx == 0:
            ...
            if setting == "none":
                system_prompt = DESCRIPTIVE_SYSTEM_PROMPT_NO_BELIEF.replace("WIDTH", str(width)).replace("HEIGHT", str(height)).replace("WINSIZE", str(window_size))
            elif setting == "zeroth":
                system_prompt = DESCRIPTIVE_SYSTEM_PROMPT_ZEROTH_BELIEF.replace("WIDTH", str(width)).replace("HEIGHT", str(height)).replace("WINSIZE", str(window_size))
            elif setting == "first":
                system_prompt = DESCRIPTIVE_SYSTEM_PROMPT_ZEROTH_AND_FIRST_BELIEF.replace("WIDTH", str(width)).replace("HEIGHT", str(height)).replace("WINSIZE", str(window_size))
...
if __name__ == "__main__":
    ...
    for setting in ["none", "zeroth", "first"]:
        ...
        with open(f"/nfs/turbo/coe-chaijy/roihn/commongrid/dataset/SFT/llava_format_pickandopen_{setting}_belief_as_input.json", "w") as fb:
